...
import { shortlistCandidates, transformToInterviewFormat, validateShortlistingPossible } from '@/lib/shortlistingLogic';
...
async function updateApplicationStatuses(jobId, shortlistedIds, rejectedIds) {
  try {
    ...
    console.error(`Error updating application statuses for job ${jobId}:`, error);
    throw error;
  }
}
    ...
    for (const candidate of shortlistedCandidates) {
      try {
        await sendShortlistNotification({
          ...
          interviewDate: new Date(new Date().getTime() + 24 * 60 * 60 * 1000).toLocaleDateString()
        });
      } catch (error) {
...
async function processJobShortlisting() {
  console.log(`[CRON] Starting job shortlisting process at ${new Date().toISOString()}`);
  ...
  try {
    ...
    for (const job of expiredJobs) {
      try {
        ...
        const existingInterview = await Interview.findOne({ jobId: job._id }).lean();
        if (existingInterview) {
          console.log(`[CRON] Interview already exists for job ${job._id}, skipping`);
        ...
        if (!validation.isValid) {
          console.log(`[CRON] Validation failed for job ${job._id}: ${validation.message}`);
        ...
        // Send notifications
        const recruiter = await jobModel.findById(job._id).populate('recruiterId', 'email fullName companyName').lean();
    ...
    console.log(`[CRON] Job shortlisting process completed at ${new Date().toISOString()}`);
